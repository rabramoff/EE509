...
We are doing this in order to convert radians to degrees.

3.	How is the atan2 function different from the atan function? 

It takes two arguments, x and y, which represents the point (x,y). The function returns the angle between the x-axis and the vector (0,0) to (x,y).

4.	What is the difference between log and log10?

Log is base e and log10 is base 10.

5.	Given a right triangle with sides x = 5 and y = 13, calculate the length of the hypotenuse (show code)

atan(opposite/adjacent) = x
hypotenuse = opposite/sin(x)
hypotenuse = opposite/sin(atan(opposite/adjacent))
```{r}
5/(sin(atan(5/13)))
```
...
Sum gives a single number (the sum of numbers 1-10) while cumsum gives you 1, 1+2, 1+2+3, ..., sum(1:9)+10
...
```{r}
dat2 = read.table("C:/Users/rose/Documents/EE509/my_frogs.csv",header=TRUE, sep = "|")
```
...
```{r}
xwomen <- c("kittypryde", "rogue", "jubilee", "emmafrost")
```
...
```{r}
  taddies <- dat[dat$tadpoles>3 & dat$tadpoles<5,]
  taddies
```
...
```{r}
barplot(tapply(dat$frogs,dat$spots,median), names.arg = c("no spots", "spots")) 
```  
  21.	Plot a histogram of blue frogs
```{r}
hist(dat[dat$color == "blue",1])
```
...
```{r}
reg.red = lm(tadpoles ~ frogs,data=subset(dat, color == "red")) 
summary(reg.red)
```
  
  b.	Make a scatter plot of this data that includes the regression line
```{r}
plot(tadpoles ~ frogs,data=subset(dat, color == "red"))
abline(reg.red)
```
...
The most complex model with all terms significant is tadpoles ~ frogs.
```{r}
reg.sink = lm(tadpoles ~ frogs+spots+color+frogs*spots+frogs*color+color*spots, data = dat)
summary(reg.sink)
``` 
```
...
```{r}
my.plot = function(x,y,color,spots){
  plot(y ~ x,
     cex=1.5,      	# increase the symbol size
...
abline(lm(y~x),col="green",		# add the regression line
     ,lwd=3)				# increase the line width
legend("topleft",
       c("Red no spot","Blue no spot","Red spots","Blue Spots"),
       pch=c(1,1,2,2),
...
dat.sub = subset(dat,tadpoles>=5)
attach(dat.sub)
my.plot(frogs,tadpoles,color,spots)
detach(dat.sub)
